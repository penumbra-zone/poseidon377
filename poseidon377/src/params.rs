...
///
/// Note: `F` must be the BLS12-377 scalar field.
pub fn rate_3<F: PrimeField>() -> Parameters<F> {
    let mds = vec![
        vec![
            F::from_str(
...
pub fn rate_4<F: PrimeField>() -> Parameters<F> {
    let mds = vec![
        vec![
            F::from_str(
                "6559848426036244266318754037733852411437289925888736621504332524252145359058",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
                "7786630835604991136508253271269363068305280772838293133030931708599899967371",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
                "7428699022136995580231146036379041992602104341879799509088961245826161691158",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
                "3947256154255559126257286466480597797613292023236275549219423063452728509522",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
                "2399773685267295765180680106329380849044669230272582160824372356850842670466",
            )
            .map_err(|_| ())
            .unwrap(),
        ],
        vec![
            F::from_str(
                "215446169755863817010549660120790827713072416229099176873396793812402925519",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
        ...
        vec![
            F::from_str(
                "8054718379289912215468126315300731290936673044823962700292400034688648279427",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
        ...
        vec![
            F::from_str(
                "7619281641515767326368641292492345327893431647676517763205359369812655123782",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
        ...
        vec![
            F::from_str(
                "2169334592295554149671817872794358151091573342620612130865918414246649429781",
            )
            .map_err(|_| ())
            .unwrap(),
            F::from_str(
