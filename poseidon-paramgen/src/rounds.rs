...
impl From<RoundNumbers> for RoundNumbersWrapper {
    fn from(value: RoundNumbers) -> Self {
        Self(value)
    }
}

impl Deref for RoundNumbersWrapper {
    type Target = RoundNumbers;
...
impl RoundNumbersWrapper {
    ...
    pub fn new<T: BigInteger>(input: &InputParameters<T>, alpha: &Alpha) -> RoundNumbers {
        ...
        for r_P in 1..400 {
            for r_F in 4..100 {
                let mut candidate = RoundNumbersWrapper(RoundNumbers { r_F, r_P });
    ...
    fn is_secure<T: BigInteger>(&self, input: &InputParameters<T>, alpha: &Alpha) -> bool {
        ...
        if self.0.r_F < RoundNumbersWrapper::statistical_attack_full_rounds(input, alpha) {
            return false;
        }
        ...
        match alpha {
            ...
            // number of rounds.
            Alpha::Exponent(_) => {
                if self.total() <= RoundNumbersWrapper::algebraic_attack_interpolation(input, alpha)
                {
                    return false;
                }
                if self.total() <= RoundNumbersWrapper::algebraic_attack_grobner_basis(input, alpha)
                {
                    return false;
                }
            }
            ...
            Alpha::Inverse => {
                if (self.0.r_F as f64 * (input.t as f64).log2()).floor() as usize + self.0.r_P
                    <= RoundNumbersWrapper::algebraic_attack_interpolation(input, alpha)
                {
                    return false;
                }
                if (self.0.r_F as f64 * (input.t as f64).log2()).floor() as usize + self.0.r_P
                    <= RoundNumbersWrapper::algebraic_attack_grobner_basis(input, alpha)
                {
                    return false;
                }
            }
